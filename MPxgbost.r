...
names(train_rent_reg)
str(train_rent_reg)
train_rent_reg <- train_rent_reg[,-c(23:27)]
train_rent_reg <- train_rent_reg[,-27]
train_rent_reg <- train_rent_reg[,-c(28:30)]
train_rent_reg <- train_rent_reg[,-1]
...
X_train = train_4 %>% select(Total_land_real_area, M_p, Total_land_gross_area, Minimum_sales_price, Total_building_area, Auction_count, Appraisal_date, addr_si) %>% as.matrix()
y_train = train_4$H_p
dtrain = xgb.DMatrix(X_train, label = y_train)
model = xgboost(data = dtrain, nround = 1000,objective="reg:linear", eval_metric="rmse", max_depth = 10, eta = 0.025, subsample = 0.9)
xgb.importance(feature_names = colnames(X_train), model) %>% xgb.plot.importance()
X_test = test_1 %>% select(-Hammer_price,-H_p) %>% as.matrix()
y_test = test_1$H_p
preds <- predict(model, X_test)
test_5$Hammer_price <- preds
test_5$Hammer_price
...
sub1
write.csv(sub1, file='try_1225xg_mp.csv', row.names=FALSE)